...
void closeStore(Store *store) {
  ...
  Record *cur, *tmp;
  HASH_ITER(hh, store->hashmap, cur, tmp) { HASH_DEL(store->hashmap, cur); };
  free(store);
}
...
char *runCommand(Store *store, char *command) {
  ...
  case 1:
    ...
    if (!value) {
      ret = malloc(strlen("UND ") + strlen(toRun->key) +
                   strlen(" not found in store!") + 2);
      strcpy(ret, "UND ");
  ...
  case 3:
    value = getValue(store, toRun->key);
    if (!value) {
      ret = malloc(strlen("ERR ") + strlen(toRun->key) +
                   strlen(" not found in store!") + 2);
      strcpy(ret, "ERR ");
      strcat(ret, toRun->key);
      strcat(ret, " not found in store!\n");
